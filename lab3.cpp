...
DWORD WINAPI thread_d(LPVOID lpParam) {
	...
	ReleaseSemaphore(semaphore_i2, 1, NULL);

	for (int i = 0; i < 3; i++) {

		WaitForSingleObject(semaphore_i2, INFINITE);
		WaitForSingleObject(lock, INFINITE);
		std::cout << 'd' << std::flush;
		ReleaseMutex(lock);
		computation();
...
DWORD WINAPI thread_f(LPVOID lpParam) {
	...
	for (int i = 0; i < 3; i++) {
		...
		ReleaseMutex(lock);
		computation();
		ReleaseSemaphore(semaphore_f2, 1, NULL);
	}
...
DWORD WINAPI thread_g(LPVOID lpParam) {
	...
	for (int i = 0; i < 3; i++) {
		...
		WaitForSingleObject(semaphore_f2, INFINITE);
		WaitForSingleObject(lock, INFINITE);
		std::cout << 'g' << std::flush;
		ReleaseMutex(lock);
		computation();
...
int lab3_init() {
	...
	semaphore_f2 = CreateSemaphore(NULL, 0, 11, NULL);
	if (semaphore_f2 == NULL) {
		std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;
	}
	...
	semaphore_i2 = CreateSemaphore(NULL, 0, 11, NULL);
	if (semaphore_i2 == NULL) {
		std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;
	}
